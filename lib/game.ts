...
export function start(r:Room,rows:number,cols:number,now:number){if(!Number.isInteger(rows)||!Number.isInteger(cols)||rows<2||rows>8||cols<2||cols>8)throw Error('行列數需為 2–8');const online=present(r,now);if(online.length<2)throw Error('至少需要 2 位在線玩家');const count=rows*cols;const cats=shuffle(Array.from({length:36},(_,i)=>i)).slice(0,Math.floor(count/2));r.deck=shuffle(cats.flatMap(x=>[x,x]));if(count%2)r.deck.splice(Math.floor(count/2),0,-1);r.rows=rows;r.cols=cols;r.matched=[];r.flipped=[];r.players=r.players.filter(p=>online.some(o=>o.id===p.id));r.players.forEach(p=>{p.score=0;p.spectator=false});r.turn=r.players[0].id;r.phase='playing';r.resolveAt=0;r.deadline=now+35000;r.round++;r.last='新的一局，開始！'}
