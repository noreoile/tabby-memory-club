...
export type Room={messages?:ChatMessage[];code:string;players:Player[];host:string;rows:number;cols:number;deck:number[];matched:number[];flipped:number[];turn:string;phase:'lobby'|'playing'|'finished';resolveAt:number;deadline:number;round:number;last:string;actions:string[]};
...
export function view(r:Room,pid:string,version:number,now:number){return {...r,players:r.players.filter(p=>!p.left||r.phase!=='lobby').map(({secret,...p})=>({...p,online:!p.left&&now-p.seen<45000})),deck:r.deck.map((v,i)=>v===-1?-1:r.flipped.includes(i)||r.matched.includes(i)?v:null),actions:undefined,you:pid,version,serverTime:now}}
