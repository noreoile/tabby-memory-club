...
import {type Room,type Player,settle,start,flip,view,sendChat,sendReaction,configure,parseItemCards} from '@/lib/game';
...
async function handle(request:Request,body:any){const db=roomDb();const now=Date.now();
 ...
 const deckSet=body.deckSet??'tabby';if(!isDeckSet(deckSet))throw Error('請選擇有效的貓咪牌組');const itemCards=parseItemCards(body.itemCards);
 const token=crypto.randomUUID()+crypto.randomUUID();const p=await player(body.name,body.avatar,token);
 for(let n=0;n<5;n++){const c=code();const r:Room={deckSet,itemCards,code:c,players:[p],host:p.id,rows:4,cols:4,deck:[],matched:[],flipped:[],turn:p.id,phase:'lobby',resolveAt:0,deadline:0,round:0,last:'等待朋友加入',actions:[]};const added=await db.prepare('INSERT OR IGNORE INTO rooms(code,state,version,expires_at) VALUES(?,?,0,?)').bind(c,JSON.stringify(r),now+86400000).run();if(added.meta.changes)return json({room:view(r,p.id,0,now),token})}throw Error('建立房間失敗，請再試一次')}
 ...
 if(body.action==='start'){if(r.host!==p.id)throw Error('只有房主可以開始');if(r.phase==='playing')throw Error('請先完成這局');const setup=r.nextSetup??{deckSet:r.deckSet??'tabby',rows:body.rows,cols:body.cols,itemCards:r.itemCards??parseItemCards(body.itemCards)};start(r,setup.rows,setup.cols,now,setup.itemCards);r.deckSet=setup.deckSet}
 else if(body.action==='configure')configure(r,p.id,body.deckSet,body.rows,body.cols,body.itemCards);
