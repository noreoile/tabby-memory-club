...
await call('start',h.token);assert.equal(room.code,code);assert.equal(room.deckSet,'abyssinian');assert.equal(room.deck.length,42);assert.equal(room.players.length,2);assert.equal(room.messages.at(-1).text,'Same room');assert.ok(room.players.every(p=>p.score===0));
console.log('PASS host-only setup, guest sync, no midround changes, completed board preserved, same-room deck switch, players/chat retained, 36 avatars');
