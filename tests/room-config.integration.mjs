...
await call('configure',h.token,{deckSet:'calico',rows:6,cols:7});assert.equal(room.deckSet,'otters');assert.deepEqual(room.deck,previousDeck);assert.deepEqual(room.players.map(p=>p.score),previousScores);
