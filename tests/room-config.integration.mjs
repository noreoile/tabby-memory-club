...
await call('configure',h.token,{deckSet:'otters',rows:2,cols:2,itemCards:['laser']},400);
...
await call('reaction',w.token,{reaction:'reaction-1',reactionId:crypto.randomUUID()});assert.equal(room.reactions.at(-1).kind,'reaction-1');
await call('read',g.token);assert.equal(room.reactions.at(-1).kind,'reaction-1','another player should receive the reaction');
...
await call('configure',h.token,{deckSet:'calico',rows:6,cols:7,itemCards:['bomb','banana','freeze']});assert.equal(room.deckSet,'otters');assert.deepEqual(room.deck,previousDeck);assert.deepEqual(room.players.map(p=>p.score),previousScores);assert.deepEqual(room.nextSetup.itemCards,['bomb','banana','freeze']);
await call('start',h.token);assert.equal(room.code,code);assert.equal(room.deckSet,'calico');assert.deepEqual(room.itemCards,['bomb','banana','freeze']);assert.equal(room.deck.length,42);assert.equal(room.players.length,3);assert.equal(room.players.find(p=>p.name==='Watcher').spectator,false);assert.equal(room.messages.at(-1).text,'Same room');assert.ok(room.players.every(p=>p.score===0));
console.log('PASS setup, chat, reactions, item cards, midgame spectating and next-round promotion');
